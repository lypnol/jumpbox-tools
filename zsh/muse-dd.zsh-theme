...
function get_hostname() {
    ...
    if [ -e /etc/update-motd.d/95-roles ]; then;
        host_role=$(/etc/update-motd.d/95-roles | awk '/Roles:/,/Tags:/' | grep -vE 'Roles:|Tags:' | head -n-1 | grep -vE 'AZ: ' | grep -Ev 'common-node|monitoring-client|dog-base|encrypted-storage|datacenter' | paste -d',' -s -)
    else;
        host_role=$(hostname)
    fi;
